...
TEST_CASE("Insert and select tiny int.")
{
    ...
    insert_stm << "INSERT INTO " << MYSQL_DATABASE << ".intergers (t, ut) VALUES (-128, 5)";
    ...
    select_stm << "SELECT t, ut FROM " << MYSQL_DATABASE << ".intergers WHERE id = " << insert_query->LastInsertId();
...
TEST_CASE("Insert and select small int.")
{
    ...
    wing::Statement insert_stm {};
    insert_stm << "INSERT INTO " << MYSQL_DATABASE << ".intergers (s, us) VALUES (-32768, 32768)";
    auto insert_query = executor.StartQuery(std::move(insert_stm), 10s).value().get();
    query_print_error(insert_query);
    REQUIRE(insert_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(insert_query->RowCount() == 1);
    ...
    wing::Statement select_stm {};
    select_stm << "SELECT s, us FROM " << MYSQL_DATABASE << ".intergers WHERE id = " << insert_query->LastInsertId();
    auto select_query = executor.StartQuery(std::move(select_stm), 10s).value().get();
    query_print_error(select_query);
    REQUIRE(select_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(select_query->RowCount() == 1);
...
TEST_CASE("Insert and select medium int.")
{
    ...
    wing::Statement insert_stm {};
    insert_stm << "INSERT INTO " << MYSQL_DATABASE << ".intergers (m, um) VALUES (-8388608, 8388608)";
    auto insert_query = executor.StartQuery(std::move(insert_stm), 10s).value().get();
    query_print_error(insert_query);
    REQUIRE(insert_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(insert_query->RowCount() == 1);
    ...
    wing::Statement select_stm {};
    select_stm << "SELECT m, um FROM " << MYSQL_DATABASE << ".intergers WHERE id = " << insert_query->LastInsertId();
    auto select_query = executor.StartQuery(std::move(select_stm), 10s).value().get();
    query_print_error(select_query);
    REQUIRE(select_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(select_query->RowCount() == 1);
...
TEST_CASE("Insert and select int.")
{
    ...
    wing::Statement insert_stm {};
    insert_stm << "INSERT INTO " << MYSQL_DATABASE << ".intergers (i, ui) VALUES (-2147483648, 2147483648)";
    auto insert_query = executor.StartQuery(std::move(insert_stm), 10s).value().get();
    query_print_error(insert_query);
    REQUIRE(insert_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(insert_query->RowCount() == 1);
    ...
    wing::Statement select_stm {};
    select_stm << "SELECT i, ui FROM " << MYSQL_DATABASE << ".intergers WHERE id = " << insert_query->LastInsertId();
    auto select_query = executor.StartQuery(std::move(select_stm), 10s).value().get();
    query_print_error(select_query);
    REQUIRE(select_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(select_query->RowCount() == 1);
...
TEST_CASE("Insert and select bigint.")
{
    ...
    wing::Statement insert_stm {};
    insert_stm << "INSERT INTO " << MYSQL_DATABASE << ".intergers (b, ub) VALUES (-9223372036854775807, 9223372036854775807)";
    auto insert_query = executor.StartQuery(std::move(insert_stm), 10s).value().get();
    query_print_error(insert_query);
    REQUIRE(insert_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(insert_query->RowCount() == 1);
    ...
    wing::Statement select_stm {};
    select_stm << "SELECT b, ub FROM " << MYSQL_DATABASE << ".intergers WHERE id = " << insert_query->LastInsertId();
    auto select_query = executor.StartQuery(std::move(select_stm), 10s).value().get();
    query_print_error(select_query);
    REQUIRE(select_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(select_query->RowCount() == 1);
...
TEST_CASE("Test Null Insert")
{
    ...
    wing::Statement insert_stm {};
    insert_stm << "INSERT INTO " << MYSQL_DATABASE << ".intergers (b, ub) VALUES (NULL, 1515151515)";
    auto insert_query = executor.StartQuery(std::move(insert_stm), 10s).value().get();
    query_print_error(insert_query);
    REQUIRE(insert_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(insert_query->RowCount() == 1);
...
TEST_CASE("Test Throw Returns un-engaged optional")
{
    ...
    wing::Statement insert_stm {};
    insert_stm << "INSERT INTO " << MYSQL_DATABASE << ".string (vc) VALUES ('TEST')";
    auto insert_query = executor.StartQuery(std::move(insert_stm), 10s).value().get();
    query_print_error(insert_query);
    REQUIRE(insert_query->QueryStatus() == wing::QueryStatus::SUCCESS);
    REQUIRE(insert_query->RowCount() == 1);
    ...
    wing::Statement select_stm {};
    select_stm << "SELECT vc FROM " << MYSQL_DATABASE << ".string WHERE id = " << insert_query->LastInsertId();
    auto select_query = executor.StartQuery(std::move(select_stm), 10s).value().get();
    query_print_error(select_query);
